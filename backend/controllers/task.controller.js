...
exports.createTask = async (req, res) => {
    try {
        ...
        const profile = await WorkerProfile.findByWorkerId(worker_id);
        ...
        await sql`
            ...
            VALUES (${uuidv4()}, ${worker_id}, 'new_task', 'New Job Request',
...
exports.getTasks = async (req, res) => {
    try {
        ...
        const tasks = await Task.findByUser(req.user.id, req.user.role, status || null);
        res.json({ tasks });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.acceptTask = async (req, res) => {
    try {
        ...
        const updated = await Task.updateStatus(task.id, 'accepted');

        const sql = getDb();
        await sql`
            INSERT INTO notifications (id, user_id, type, title, message, data) 
            VALUES (${uuidv4()}, ${task.homeowner_id}, 'task_accepted', 'Job Accepted',
...
exports.startTask = async (req, res) => {
    try {
        ...
        const updated = await Task.updateStatus(task.id, 'in_progress');
        res.json({ task: updated });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.completeTask = async (req, res) => {
    try {
        ...
        const updated = await Task.updateStatus(task.id, 'completed');
        await WorkerProfile.incrementJobs(req.user.id);
        ...
        await sql`
            INSERT INTO notifications (id, user_id, type, title, message, data) 
            VALUES (${uuidv4()}, ${task.homeowner_id}, 'task_completed', 'Job Completed',
