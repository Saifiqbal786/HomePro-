...
exports.createReview = (req, res) => {
    try {
        ...
        const existing = Review.existsForTask(task_id);
        if (existing) return res.status(409).json({ error: 'Review already submitted for this task.' });

        const review = Review.create({ homeowner_id: req.user.id, worker_id, task_id, rating, comment });

        // Update worker rating
        WorkerProfile.updateRating(worker_id);

        // Notify worker
        getDb().prepare(`INSERT INTO notifications (id, user_id, type, title, message, data) VALUES (?, ?, ?, ?, ?, ?)`)
            .run(uuidv4(), worker_id, 'new_review', 'New Review',
...
exports.getWorkerReviews = (req, res) => {
    try {
        const { limit, offset } = req.query;
        const reviews = Review.findByWorker(req.params.id, parseInt(limit) || 20, parseInt(offset) || 0);
        res.json({ reviews });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
